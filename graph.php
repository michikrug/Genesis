...
class graph {
  ...
  var $parameter         =   array(        // input parameters
    ...
    'path_to_fonts'      => '',      	   // path to fonts folder. don't forget *trailing* slash!!
                                           //   for WINDOZE this may need to be the full path, not relative.

    'title'              => 'Graph Title', // text for graph title
    'title_font'         => 'verdana.ttf',   // title text font. don't forget to set 'path_to_fonts' above.
    'title_size'         =>  12,           // title text point size
    'title_colour'       => 'white',       // colour for title text
    ...
    'label_font'         => 'verdana.ttf', // label text font. don't forget to set 'path_to_fonts' above.
    'label_colour'       => 'grayCC',      // label text colour
    ...
    'inner_border'       => 'grayCC',       // colour of border around actual graph, or 'none'.
    ...
    'y_resolution_right' =>  1,            // ... same for right hand side
    'y_decimal_right'    =>  0,            // ... same for right hand side
    'x_resolution'       =>  1,            // only used if x axis is numeric.
    ...
    'shadow_offset'      =>  2,            // draw shadow at this offset, unless overidden by data parameter.
    'shadow'             => 'none',      // 'none' or colour of shadow.
    ...
    'axis_font'          => 'verdana.ttf', // axis text font. don't forget to set 'path_to_fonts' above.
    'axis_size'          =>  9,            // axis text font size in points
    'axis_colour'        => 'grayCC',      // colour of axis text.
    ...
    'y_grid'             => 'dash',        // grid lines. set to 'line' or 'dash'...
    'x_grid'             => 'dash',        //   or if set to 'none' print nothing.
    'grid_colour'        => 'gray99',      // default grid colour.
    ...
    'legend_font'        => 'verdana.ttf',   // legend text font. don't forget to set 'path_to_fonts' above.
...
function draw_set($order, $set, $offset) {
  ...
  //
  if ($offset) {
    if (isset($this->y_format[$set]['shadow'])) $colour = $this->y_format[$set]['shadow'];
    else                                        $colour = $this->parameter['shadow'];
  } else {
...
function draw_data() {
  ...
  foreach ($this->y_order as $order => $set) {
    ...
    if (isset($this->y_format[$set]['shadow_offset'])) $offset =  $this->y_format[$set]['shadow_offset'];

    $colour = $this->parameter['shadow'];
    if (isset($this->y_format[$set]['shadow'])) $colour =  $this->y_format[$set]['shadow'];

    if (isset($colour)) if ($colour != 'none') $this->draw_set($order, $set, $offset);
  }
...
function init_x_axis() {
  ...
  if ($this->parameter['x_axis_gridlines'] == 'auto') { // auto means text based x_axis, not numeric...
    ...
    for ($i = 0; $i < $this->calculated['x_axis']['num_ticks']; $i++) { // calculate x axis text sizes
      $value = number_format($start, $this->parameter['x_decimal'], $this->parameter['decimal_point'], $this->parameter['thousand_sep']);
...
function print_TTF($message) {
  ...
  $font      = realpath($this->parameter['path_to_fonts'].$message['font']);
...
function init_colours() {
  ...
  $this->colour['grayBB']   = ImageColorAllocate ($this->image, 0xBB, 0xBB, 0xBB);
  $this->colour['grayAA']   = ImageColorAllocate ($this->image, 0xAA, 0xAA, 0xAA);
  ...
  $this->colour['bau']      = ImageColorAllocate ($this->image, 0x33, 0x99, 0xFF);
  $this->colour['evo']      = ImageColorAllocate ($this->image, 0x33, 0xCC, 0x66);
  $this->colour['prod']	    = ImageColorAllocate ($this->image, 0xFF, 0xCC, 0x00);
...
function output() {
  if ($this->debug) { // for debugging purposes.
    ...
    if ($this->parameter['file_name'] == 'none' || empty($this->parameter['file_name'] )) {
	  ...
	  imagecolortransparent ($this->image, $this->colour['black']);
	  ImagePNG($this->image);
          break;
      }
    } else {
