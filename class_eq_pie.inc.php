...
class eq_pie {
	...
	function MakePie($filename, $pieWidth, $pieHeight, $ShadowDistance, $pieBackgroundColor, $EQpieData, $legend)
	{
		...
		$transCol = imagecolorallocate($finalPie, 0, 0, 0);
		...
		if ($legendWidth > 0) {
			...
			$textColor = ImageColorAllocate($pie, '255', '255', '255');
			$boxColor = ImageColorAllocate($pie, '0', '0', '0');
		...
		imagetruecolortopalette($finalPie, false, 255);
		$transCol = imagecolortransparent($finalPie, $transCol);
