...
if (time() - $inhalte_game["zeit"] >= 180 && $inhalte_game["aktiv"] == 0) {
	...
	$ze = time() - 300;
	$result_check = mysql_query("SELECT * FROM genesis_basen WHERE resszeit>'0' AND resszeit<'$ze' ORDER BY resszeit LIMIT 100");
	...
	$result_check = mysql_query("SELECT * FROM genesis_aktionen WHERE endzeit<='" . time() . "' order by endzeit limit 30");
	while ($inhalte_check = mysql_fetch_array($result_check, MYSQL_ASSOC)) {
